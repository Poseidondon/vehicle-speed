...
def process(model, video_path, markup_path, vid_stride=0, tracker="bytetrack.yaml"):
    ...
    pbar = tqdm(total=TOTAL_N_FRAMES)
    while cap.isOpened():
        ...
        if success:
            ...
            result = model.track(frame, persist=True, tracker=tracker, iou=0.3, conf=0.4, verbose=False)
            ...
            print(f"FRAME {frame_n}/{TOTAL_N_FRAMES}: break")
